    ...
    bool BPLUSTREE_TYPE::Coalesce(
        ...
        if (isLeftSibling) {
            ...
            if (!buffer_pool_manager_->DeletePage(nodeId)) {
                throw Exception("[delete failed, pin_count != 0]");
            }
 ...
 * happend
 * 案例1：当您删除根页面中的最后一个元素，但根页面仍然有最后一个孩子
 ...
 */
    ...
    bool BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node, Transaction *transaction) {
        ...
        UpdateRootPageId();
        Page *new_root_p = GetPage(root_page_id_, EXCEPTION_INFO);
        BPlusTreePage *new_root = reinterpret_cast<BPlusTreePage *>(new_root_p->GetData());
 ...
 */
    ...
    INDEXITERATOR_TYPE BPLUSTREE_TYPE::Begin() {
        KeyType fKey;
        auto leaf_begin = FindLeafPage(fKey, OperationType::GET, nullptr, true);
        return INDEXITERATOR_TYPE(leaf_begin, 0, buffer_pool_manager_);
    }
 ...
 */
    ...
    INDEXITERATOR_TYPE BPLUSTREE_TYPE::Begin(const KeyType &key) {
        auto leaf_begin = FindLeafPage(key, OperationType::GET);
        int index_begin = 0;
        //
        if (leaf_begin != nullptr) {
            int index = leaf_begin->KeyIndex(key, comparator_);
            if (leaf_begin->GetSize() > 0 && index < leaf_begin->GetSize() && comparator_(key, leaf_begin->GetItem(index).first) == 0) {
                index_begin = index;
            }
            else {
                index_begin = leaf_begin->GetSize();
            }
        }
        return INDEXITERATOR_TYPE(leaf_begin, index_begin, buffer_pool_manager_);
    }
 ...
 */
    ...
    B_PLUS_TREE_LEAF_PAGE_TYPE *BPLUSTREE_TYPE::FindLeafPage(const KeyType &key,
        ...
        while (!bp->IsLeafPage()) {
            ...
            page_id_t next_id;
            if (leftMost) {
                next_id = internalPage->ValueAt(0);
            } else {
                next_id = internalPage->Lookup(key, comparator_);
            }
            BPlusTreePage *lastBp = bp;
            Page *finalPage = page;
            page = GetPage(next_id, EXCEPTION_INFO);
            if (op == OperationType::GET) {
                page->RLatch();
            } else {
                page->WLatch();
            ...
            if (transaction != nullptr) {
                if (op == OperationType::GET) {
                    UnLatchAndUnpinPageSet(transaction, op);
                ...
                assert(op == OperationType::GET);
                finalPage->RUnlatch();
                if (lastBp->IsRootPage()) {
                    root_id_mutex_.unlock();
                }
                buffer_pool_manager_->UnpinPage(finalPage->GetPageId(), false);
            }
